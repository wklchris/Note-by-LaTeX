...
\subsubsection{边注}
\LaTeX 支持的边注命令\latexline{marginpar}不会进行编号。其必选参数表示页右注释的内容；可选参数表示如果边注出现在偶数页，则在页左显示的内容。例如：
\begin{latex}
这一行有边注\marginpar[左--]{--右}
\end{latex}
...
\hline
环境 & 层 & Counter & \multicolumn{1}{l}{默认} & Label & \multicolumn{1}{l}{默认}	\\
\hline
\multirow{4}*{\envi{enumerate}} & 1 & \latexline{theenumi} & arabic\{enumi\} & \latexline{labelenumi} & theenumi. \\
& 2 & \latexline{theenumii} & alph\{enumi\} & \latexline{labelenumi} & (theenumii) \\
& 3 & \latexline{theenumiii} & roman\{enumi\} & \latexline{labelenumiii} & theenumiii. \\
& 4 & \latexline{theenumiv} & Alph\{enumiv\} & \latexline{labelenumiv} & theenumiv. \\
\hline
