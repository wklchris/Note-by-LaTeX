...
其中命令\latexline{char[num]}是一个特殊的命令，用于输出USCII码对应的字符；92对应的即反斜杠。你也可以用\latexline{char`}后加字符的方式输出你想输出的命令，但需要包裹在\latexline{texttt}或者\latexline{\char`\\ttfamily}内。如果想输出的字符是保留字符，需要再加一个反斜杠。
\begin{verbatim}
\texttt{\char`~} % 输出一个波浪线
\texttt{\char`\\} % 输出保留字反斜杠
\texttt{\char`@} % 实际上可直接输入@
...
\subsection{西文字体}
在\xelatex 编译下，一般使用\pkg{fontspec}宏包来选择西文字体。注意：该宏包可能会明显增加编译所需的时间。
\begin{latex}
\usepackage{fontspec}
...
然后在document环境内第一行，写上：\latexline{maketitle}，就能产生一个简易的封面。其中\latexline{title}和\latexline{author}是必须定义的，\latexline{date}如果省略会自动以编译当天的日期为准，格式形如：January 1, 1970。 如果你不想显示日期，可以写\latexline{date\{\}}。
...
\subsection{目录}
目录在大纲的基础上生成，使用命令\latexline{tableofcontents}即可插入目录。目录在加载了hyperref宏包后，可以实现点击跳转的功能。你可以通过重定义命令更改\latexline{contentsname}，即“目录”的标题名。
\begin{latex}
\renewcommand{\contensname}{目录}
\end{latex}

你也可以插入图表目录，分别是\latexline{listoffigures}, \latexline{listoftables}. 通过重定义\latexline{listfigurename}和\latexline{listtablename}可以更改图表目录的标题。

目录的深度自定义需要借助\pkg{titletoc}宏包，参考\hyperref[sec:titletoc]{这一节}。
...
如果在同一页内需要分栏与单栏并存（比如摘要跨栏，正文分栏），或者想要分三栏，请参考\pkg{multicol}宏包。

\section{文档拆分}
\label{sec:include}
文档拆分很简单，只需要把多个tex文件放在一起，然后再主文件中使用\latexline{input\{filename.tex\}}或\latexline{include\{filename\}}命令，不写扩展名默认为.tex。两者区别在于\latexline{include}命令将会另起一页再读取文件，并很好地支持大纲编号。
...
相反，有些并非句末标点的情况会被识别为句末标点，这时候需要在标点后插入一个\latexline{\textvisiblespace}或者\latexline{\char`~}来缩小间距；区别在于前者允许断行，后者不允许。
...
\pkg{amsmath}宏包允许\latexline{DeclareMathOperator}命令自定义基本函数；如果命令带星号，则可以带上下限参数。
...
\subsection{分式、根式与堆叠}
分式使用\latexline{frac}命令。或者\pkg{amsmath}宏包支持的\latexline{dfrac}命令来强制获得行间公式大小的分数，\latexline{tfrac}则强制获得行内公式大小的分数：
...
划线命令使用\latexline{underline}和\latexline{overline}，水平括号使用brace代替line：
...
还有一个很强大的堆叠放置命令\latexline{sideset}：
...
还有就是\latexline{cdots}命令。\pkg{hmath}宏包还提供了一种省略号\latexline{adots}：$\adots$. 或许什么时候需要使用呢？
...
\subsection{二项式与定理}
二项式可能需要借助\pkg{amsmath}宏包的\latexline{binom}命令：

\begin{codeshow}
  $\mathrm{C}_n^k=\binom{n}{k}$
\end{codeshow}

\RED{在使用下述部分内容时，请加载\pkg{amsthm}宏包}。
...
使用\latexline{left}或者\latexline{right}能够使定界符自适应式子的高度：
...
其中\latexline{vphantom}命令用于输出一个高度虚位，使得第二行的自适应定界符与第一行同等大小。
...
\subsection{制表位*}
制表位使用\envi{tabbing}环境，需要指出，这是一个极其容易造成坏箱的环境。一个丑陋的例子：
\begin{latex}
\begin{tabbing}
...
在\LaTeX 默认的编号列表中，编号样式按照：1.$\rightarrow$(a)$\rightarrow$i.$\rightarrow$A的顺序嵌套。你可以通过计数器命令来指定编号样式，不过要额外加上一个星号，比如\latexline{\char`\\arabic*}表示阿拉伯数字。你也可以在\pkg{ctex}宏包被调用（包括ctex文档类被使用）时，在导言区加入：
\begin{latex}
