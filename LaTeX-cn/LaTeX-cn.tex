...
\begin{feae}
  ...
  \item 本手册使用的编辑器为\TeX Studio，而非之前的商业软件WinEdt. 这使得学习\LaTeX 的门槛更低。当然了，你有权使用任何编辑器，我并不是说\TeX Studio就是最好的。
\end{feae}
...
\item[\char92caption] 与原生tabular的该命令一致。如果你不想显示表格编号，使用带星的该命令；如果你不想让其加入表格目录，请在可选参数中留空\latexline{\\caption[]{...}}。
...
longtable宏包支持的表格可选参数是clr，而不能使用tabular中的t或b. 此外，\RED{longtable中的跨列可能需要编译多次（有时多达4次）才能正常显示。}最后给出一个例子：
