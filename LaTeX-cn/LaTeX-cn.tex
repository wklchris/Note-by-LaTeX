...
以上各级别在\LaTeX 内部以“深度”参数作为标识。第一级别part的深度是$-1$，以下级别深度分别是$0,1,\ldots$，类推。注意到由于article文档类缺少chapter大纲，因此chapter以下的深度与book或report文档类会有不同。\dpar
...
各参数的含义是：
\begin{inlinee}
\item \textit{linenum:} （可选）图片所占行数，一般不指定。
\item \textit{place:} 图片在文字段中的位置——R, L, I, O分别代表右侧、左侧、近书脊、远书脊。
\item \textit{overhang:} （可选）允许图片超出页面文本区的宽度，默认是0pt。在该项可以使用\latexline{\\width}代替图片的宽度，填入\latexline{\\width}将允许把图片全部放入页面区域。
\item \textit{picwidth:} 指定图片的宽度，默认情形下图片的宽度会自动调整。
\end{inlinee}
...
\item 在tabular环境内部，命令"\&"用于把光标跳入该行下一列的单元格。每行的最后请使用两个反斜杠命令跳入下一行。命令\latexline{\\hline}或\latexline{\\cline}不能算作一行，因此它们后面没有附加换行命令。
\item 在tabular环境内部，跨列命令\latexline{\\multicolumn{number}{format}{text}}用于以format格式合并该行的number个单元格，并在合并后的单元格中写入文本text。如果一行有了跨列命令，请注意相应地减少"\&"的数量。
\end{feai}
...
\section{抄录与代码环境}
抄录使用\verb|\verb|命令或者verbatim(*)环境。
...
\section{特殊符号}
这里介绍hologo宏包，可以输出许多有用的\TeX 家族标志。
