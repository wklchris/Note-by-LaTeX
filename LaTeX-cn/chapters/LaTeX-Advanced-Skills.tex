...
该环境也支持使用可选参数，用法类似原生 \latexline{newcommand} 命令．但是，如果你只有一个参数，而且它是可选的，请使用 \latexline{DeclareTCBListing} 命令（需要用 \latexline{tcbuselibrary} 命令 加载 \texttt{xparse} 库）以避免可能的问题，例如：
\begin{latex}
% 缺省值为空，表示不指定方括号参数则传入空到#1
% 也可以传入，例如：\begin{enviname}[colback=yellow]
\DeclareTCBListing{enviname}{ O{} }{listing only,#1}
\end{latex}
