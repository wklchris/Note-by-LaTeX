...
不同位置放置文本的场合，使用 \tikzkw{auto} 选项，并设定 \texttt{left} 或者 \texttt{right} 参数；选项 \tikzkw{swap} 则允许你将其文本放在对称位置．注意：这两个选项\RED{只对沿路径的放置有效}．
\begin{tikzshow}[scale=.5, every node/.style={circle}]
\foreach \x in {1,...,4} {
