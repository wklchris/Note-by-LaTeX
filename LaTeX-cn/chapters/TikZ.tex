...
\subsection{点的基本命令}
如需显式地绘制点，使用 \latexline{node} 命令，或者 \latexline{draw} 命令的 \tikzkw{node} 选项．选项 \tikzkw{shape} 用于指定点的绘制方式．
\begin{tikzshow}
\node at (0,0) [shape=circle, draw] {a};
\node at (1,0) [rectangle, draw, fill=red] {};
\end{tikzshow}
选项 \tikzkw{shape} 还可赋值为 \texttt{coordinate}，这样在点之间连线时会从点中心开始绘制，而不是从点的边框上．
...
旁置文本可用上一节的语法画另一个点来实现，但 \tikzkw{label} 选项更简洁；对同一个点应用多个 \tikzkw{label} 标签也是允许的．语法是 \texttt{角度\mbox{:}文本}．
\begin{tikzshow}[every node/.style={draw, circle}]
\draw node (a) at (0,0) {a} 
node[label={[red]right:$b'$}] (b) at (0,1) {b} 
node[label=120:$c'$, label=below:$c''$] (c) at (1.5,1) {c};
\end{tikzshow}
上例中甚至给 \tikzkw{label} 传入了颜色参数．
