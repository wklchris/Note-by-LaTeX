...
\end{tikzshow}
圆弧使用极坐标参数绘制往往更加简便，参考\secref{subsec:coord}．
...
\subsubsection{隐式指定}
路径中点的位置可以隐式（implicitly）指定，放在 \texttt{--} 命令与要连接的点之间即可．只有隐式指定的点才会继承全局参数．例子\cite{tikzmanual}：
\begin{tikzshowenvi}
