...
\subsection{点的基本命令}
如需显式地绘制点（即带有几何属性的点），使用 \latexline{node} 命令，或者 \latexline{path} 命令的 \tikzkw{node} 选项．选项 \tikzkw{shape} 用于指定点的绘制方式．
\begin{tikzshow}
...
\end{scope}
\end{tikzshow}
上例中还使用了 \pkg{fit} 库，用来创建一个“遮盖”点 b 和点 c 的背景点——它被 \tikzkw{fit} 命令处理成图中的大小．

\subsection{装饰*（decorations）}
\label{subsec:deco}
最简单的一种装饰是画蛇形线\cite{tikzmanual}（snake 选项需要 \pkg{decorations.pathmorphing} 库）：
\begin{tikzshow}
...
\end{tikzshow}
大部分的参数都比较好理解．\tikzkw{amplitude} 控制波动的强弱，\tikzkw{segment length} 控制一个周期的长度，\tikzkw{post length} 控制提前（相比于终点）多远结束装饰．
